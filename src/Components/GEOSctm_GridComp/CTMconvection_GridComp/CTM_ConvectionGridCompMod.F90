...
!                       ------------------------
      ...
      call rcEsmfReadLogical(convConfigFile, det_ent, &
     &               "det_ent:", default=.false., rc=STATUS )
      VERIFY_(STATUS)

      call rcEsmfReadLogical(convConfigFile, do_downdraft, &
...
!  ---
      ...
      allocate(cellArea(i1:i2,j1:j2),            STAT=STATUS); VERIFY_(STATUS)
