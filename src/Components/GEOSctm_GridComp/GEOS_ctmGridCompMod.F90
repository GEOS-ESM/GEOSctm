...
! !LOCAL VARIABLES:
      ...
      IF (CTM_STATE%do_ctmAdvection) THEN
         ...
         call Pack_Chem_Groups( GIM(CTM_STATE%ADV3) )  ! Prepare to transport chemical families
      ...
      IF (CTM_STATE%do_ctmConvection) THEN
         ...
         !----------------------------------
         call Initialize_IncBundle_run(GIM(CTM_STATE%CONV), EXPORT, MTRIincCTM, __RC__)
         ...
         !--------------------------------------------
         call Compute_IncBundle(GIM(CTM_STATE%CONV), EXPORT, MTRIincCTM, STATE, __RC__)
      END IF
      ...
      IF (CTM_STATE%do_ctmDiffusion) THEN
         ...
         !----------------------------------
         call Initialize_IncBundle_run(GIM(CTM_STATE%DIFF), EXPORT, TRIincCTM, __RC__)
         ...
         !--------------------------------------------
         call Compute_IncBundle(GIM(CTM_STATE%DIFF), EXPORT, TRIincCTM, STATE, __RC__)
      END IF
