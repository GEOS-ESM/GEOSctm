...
##################################################################
#                Do the conversion with GEOSctm.x
##################################################################

       @  NPES = $NX * $NY

echo "Running GEOSctm.x to do the conversion ..."

set LOGFILE = "$CNVDIR/GEOSctm.log"
/bin/rm $LOGFILE >& /dev/null

# Assume ctm_setup set these properly for the local platform
/bin/rm -f EGRESS
@SETENVS
$RUN_CMD $NPES ./GEOSctm.x >& $LOGFILE
if( -e EGRESS ) then
   set rc = 0
else
   set rc = -1
...
if ( $rc != 0 ) then
   /bin/cat $LOGFILE
   echo " "
   echo "GEOSctm.x failed and returned with exit code $rc"
   exit $rc
else
